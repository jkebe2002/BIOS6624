...
selection_diag <- function(model, true_var, true_coef, X, y, method, case) {
  ...
  #instantiate biases and coverage and type2 error and true positive for all true variablsse
  bias <- rep(0, length(true_var))
  coverage <- rep(0, length(true_var))
  type2 <- rep(0, length(true_var))
  trueP <- rep(0, length(true_var))
...
#takes lm object
  ...
  # Set alpha to .5 for elastic net model...
...
###########################################
# Run the simulation using parallization: #
###########################################
